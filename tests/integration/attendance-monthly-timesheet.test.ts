...
test("P6B locks local-date segments and Payroll freezes the exact six buckets", async () => {
  await withRollback(async (transaction) => {
    ...
    const materialized = await materializeAttendanceP2DayInTransaction({
      context: resolutionContext(fixture),
      membershipId: fixture.membership.id,
      workDate,
    }, transaction);
    assert.ok(materialized.finalResult);
    const finalResult = materialized.finalResult;
