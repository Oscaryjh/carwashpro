...
import { GET as getEmployeePerformance } from "../../src/app/api/employee-performance/route";
...
test("real Staff cookie API enforces own-data, same-origin, logout and private cache boundaries", async () => {
  const f = await staffPerformanceFixture(db);
  await staffCash(db, f);
  const request = (path: string, token?: string, extraHeaders: Record<string, string> = {}) =>
    new Request(`http://localhost:3000${path}`, { headers: {
      ...(token ? { cookie: `${EMPLOYEE_SESSION_COOKIE}=${encodeURIComponent(token)}` } : {}),
      ...extraHeaders,
    } });
  const own = await getEmployeePerformance(request("/api/employee-performance?view=mine&year=2026&month=8", f.sessions[1].token));
  assert.equal(own.status, 200);
  assert.match(own.headers.get("cache-control") ?? "", /private, no-store/);
  const body = await own.json();
  assert.equal(body.personal.annual.total, 5000);
  assert.ok(!JSON.stringify(body).includes(f.members[0].fullName));

  const member = await getEmployeePerformance(request(`/api/employee-performance?view=member&member=${f.members[0].id}`, f.sessions[1].token));
  assert.equal(member.status, 403);
  assert.equal((await member.json()).error.code, "PERFORMANCE_SCOPE_DENIED");
  assert.equal((await getEmployeePerformance(request("/api/employee-performance?view=mine"))).status, 401);
  assert.equal((await getEmployeePerformance(request("/api/employee-performance?view=mine", f.sessions[1].token,
    { origin: "https://evil.example" }))).status, 400);

  await db.employeeSession.update({ where: { id: f.sessions[1].context.sessionId }, data: { revokedAt: new Date() } });
  const revoked = await getEmployeePerformance(request("/api/employee-performance?view=mine", f.sessions[1].token));
