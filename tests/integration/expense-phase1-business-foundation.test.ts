...
test("manual Expense lifecycle, payment, dashboard, receipt and recurring generation are safe", async () => {
  ...
  const paid = await createBusinessExpense({ actor: ownerActor, amount: "1500.00", branchId: branch.id, businessId: business.id, categoryId: marketing.id, description: "Local QA Meta campaign", desiredStatus: "CONFIRMED", expenseDate: "2026-08-11", operationKey: `EXPENSE:CREATE:PAID:${token}:0001`, payeeName: "Meta", paymentDate: "2026-08-11", paymentMethod: "CARD", paymentSource: "COMPANY_CARD", paymentReference: "LOCAL-CARD-REF", paymentStatus: "PAID", receipt: { bytes: png, claimedMimeType: "image/png", originalFileName: "meta receipt.png" } }, prisma, store);
  ...
  const paidReplay = await createBusinessExpense({ actor: ownerActor, amount: "1500.00", branchId: branch.id, businessId: business.id, categoryId: marketing.id, description: "Local QA Meta campaign", desiredStatus: "CONFIRMED", expenseDate: "2026-08-11", operationKey: `EXPENSE:CREATE:PAID:${token}:0001`, payeeName: "Meta", paymentDate: "2026-08-11", paymentMethod: "CARD", paymentSource: "COMPANY_CARD", paymentReference: "LOCAL-CARD-REF", paymentStatus: "PAID", receipt: { bytes: png, claimedMimeType: "image/png", originalFileName: "meta receipt.png" } }, prisma, store); assert.equal(paidReplay.id, paid.id); assert.equal(store.putCount, 1);
  ...
  const payRace = await Promise.allSettled(["A", "B"].map((suffix) => markBusinessExpensePaid({ actor: managerActor, businessId: business.id, expenseId: unpaid.id, expectedRevision: unpaid.revision, operationKey: `EXPENSE:PAY:${token}:${suffix}`, paymentDate: "2026-08-12", paymentMethod: "BANK_TRANSFER", paymentSource: "BANK_ACCOUNT", paymentReference: "LOCAL-ELECTRIC" }, prisma)));
