-- Keep database reconciliation aligned with the payroll component domain:
-- statutory component lines are represented for traceability, while employee
-- statutory deductions are subtracted through the frozen payroll-entry fields.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM("amount") FILTER (
      WHERE "type" = 'DEDUCTION' AND "source_type" <> 'STATUTORY'
    ), 0),
    COALESCE(SUM("amount") FILTER (
