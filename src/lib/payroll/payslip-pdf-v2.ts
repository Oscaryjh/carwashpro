...
function drawTitle(pdf: PdfCanvas, run: PdfRun) {
  pdf.text("PAYSLIP", PAGE_MARGIN, pdf.y, 20, { bold: true, color: COLORS.tealDark });
...
function drawPaySummary(pdf: PdfCanvas, entry: PdfEntry) {
  const height = 60;
  ...
  const summaries = [
    ...
    ["TOTAL DEDUCTIONS", money(
      ...
      entry.lindung24Employee + entry.pcb + entry.cp38,
    )],
    ["NET PAY", money(entry.netPay)],
  ];
