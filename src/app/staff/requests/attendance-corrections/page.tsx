...
export default async function StaffAttendanceCorrectionQueuePage({
    ...
    return (
      <section className="staff-attendance-approval-page">
        ...
        <div className="staff-page-card staff-approval-empty" role="alert">
          ...
          <span>Attendance review is available only to an authorized manager in this workplace.</span>
  ...
  return (
    <section className="staff-attendance-approval-page" aria-labelledby="attendance-review-heading">
      <Link className="staff-page-back" href="/staff/requests">← Requests</Link>
      <header className="staff-approval-header">
        <div>
          <p className="staff-kicker">TEAM ATTENDANCE</p>
          <h1 id="attendance-review-heading">Attendance</h1>
          <p>Review missing punches and submitted time corrections in your authorized branches.</p>
        </div>
        <span>{queue.totalActionable} need attention</span>
      </header>
      ...
      <div className="staff-attendance-approval-list">
          ...
          return (
            <article className="staff-attendance-approval-card" id={`correction-${item.id}`} key={item.id}>
              <header>
                ...
                <div>
                  <small>Missing punch · {item.branch.name} · {formatWorkDate(workDate)}</small>
          ...
          return (
            <article className="staff-attendance-approval-card" id={`correction-${item.id}`} key={item.id}>
              <header>
                <span className="staff-approval-domain attendance">AT</span>
                <div>
                  <small>Attendance correction · {item.branch.name} · {formatWorkDate(item.attendanceSession.workDate)}</small>
                  <strong>{item.employee.fullName}</strong>
        ...
        {!queue.totalActionable ? (
          <div className="staff-page-card staff-approval-empty">
            <strong>No attendance items need your review</strong>
            <span>New employee submissions in your authorized branches will appear here.</span>
          </div>
        ) : null}
      </div>
