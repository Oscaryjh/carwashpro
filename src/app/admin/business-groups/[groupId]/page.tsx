      ...
      <section className="content">
        ...
        <section className="panel">
          ...
          {group.status === "ACTIVE" && eligibleBusinesses.length ? (
            <BusinessGroupActionForm action={addBusinessToGroupAction} className="inline-form">
              <input type="hidden" name="groupId" value={group.id} />
              ...
              <tbody>
                {group.members.map((member) => (
                  <tr key={member.id}>
                    ...
                    <td>{group.status === "ACTIVE" && member.status === "ACTIVE" ? <BusinessGroupActionForm action={removeBusinessFromGroupAction}><input type="hidden" name="groupId" value={group.id} /><input type="hidden" name="memberId" value={member.id} /><button className="danger-button" type="submit">Remove</button></BusinessGroupActionForm> : null}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="panel">
          ...
          {group.status === "ACTIVE" && eligibleUsers.length ? (
            <BusinessGroupActionForm action={grantBusinessGroupUserAction} className="field-grid">
              <input type="hidden" name="groupId" value={group.id} />
          ...
          {group.users.length ? (
            <table className="table compact-table"><thead><tr><th>User</th><th>Role</th><th>Scope</th><th>Status</th><th>Granted</th><th /></tr></thead><tbody>{group.users.map((grant) => <tr key={grant.id}><td><strong>{grant.user.name}</strong><div className="muted">{grant.user.email ?? "No login email"}</div></td><td>{grant.role}</td><td>{grant.accessScope === "ALL_GROUP_BUSINESSES" ? "All group businesses" : grant.businessAccesses.map((access) => access.business.name).join(", ")}</td><td><span className={`status ${grant.status.toLowerCase()}`}>{grant.status.toLowerCase()}</span></td><td>{grant.grantedAt.toLocaleString()}</td><td>{group.status === "ACTIVE" && grant.status === "ACTIVE" ? <BusinessGroupActionForm action={revokeBusinessGroupUserAction}><input type="hidden" name="groupId" value={group.id} /><input type="hidden" name="groupUserId" value={grant.id} /><button className="danger-button" type="submit">Revoke</button></BusinessGroupActionForm> : null}</td></tr>)}</tbody></table>
