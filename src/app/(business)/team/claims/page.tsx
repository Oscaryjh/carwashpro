...
type Props = { searchParams: Promise<{ status?: string; employee?: string; type?: string; message?: string; manage?: string; category?: string; newCategory?: string }> };

const statusLabels: Record<string, string> = {
  SUBMITTED: "Pending review", APPROVED: "Approved", PARTIALLY_APPROVED: "Partially approved", REJECTED: "Rejected", WITHDRAWN: "Withdrawn",
  AWAITING_CHANNEL: "Awaiting reimbursement", OUTSIDE_PAYROLL_PENDING: "Payment pending", OUTSIDE_PAYROLL_PAID: "Paid separately",
  PAYROLL_LINKED: "Added to payroll", PAYROLL_SETTLED: "Paid through payroll", CANCELLED: "Cancelled", PENDING: "Pending",
};
...
export default async function ClaimsPage({ searchParams }: Props) {
  ...
  const attentionCount = submittedCount + channelCount + paymentCount;
  const hasFilters = Boolean(params.employee || params.status);
  const activeFilterCount = Number(Boolean(params.employee)) + Number(Boolean(params.status));
  ...
  return (
    <main className={styles.page}>
      ...
      {params.message ? <div className={params.type === "error" ? styles.error : styles.success}>{friendlyMessage(params.message)}</div> : null}

      <section className={styles.summary} aria-label="Claims requiring attention">
        <article className={submittedCount ? styles.summaryActive : undefined}><span>Pending approval</span><strong>{submittedCount}</strong><small>Claims waiting for a decision</small></article>
        <article className={channelCount ? styles.summaryActive : undefined}><span>Choose reimbursement</span><strong>{channelCount}</strong><small>Approved claims need a payment method</small></article>
        <article className={paymentCount ? styles.summaryActive : undefined}><span>Complete payment</span><strong>{paymentCount}</strong><small>Direct reimbursements still unpaid</small></article>
      </section>
      ...
      <section className={styles.panel}>
        <div className={styles.panelTitle}>
          <div><p className={styles.eyebrow}>CLAIM REQUESTS</p><h2>Requests</h2><span>{claims.length} result{claims.length === 1 ? "" : "s"}</span></div>
          <ClaimsFilterControls employee={params.employee} status={params.status} activeFilterCount={activeFilterCount} />
        </div>
        ...
        <div className={styles.claims}>{claims.length ? claims.map((claim) => {
          ...
          return <article key={claim.id} className={styles.claim}>
            <details className={styles.claimDisclosure}>
              <summary className={styles.claimRow}>
                ...
                <b className={styles.status} data-status={claim.status}>{statusLabels[claim.reimbursement?.status ?? claim.status] ?? humanize(claim.status)}</b>
                ...
                {claim.reimbursement ? <section className={styles.reimbursement}>
                  <div className={styles.reimbursementHeader}>
                    <div>
                      ...
                      <h3>Choose how to reimburse</h3>
                      <span>Select one payment method for this approved claim.</span>
                    </div>
                    <div className={styles.reimbursementSummary}>
                      <span><small>Approved</small><strong>RM {claim.reimbursement.amount}</strong></span>
                      <b>{claim.reimbursement.status === "AWAITING_CHANNEL" ? "Action required" : statusLabels[claim.reimbursement.status] ?? humanize(claim.reimbursement.status)}</b>
                    </div>
                  </div>
                  ...
                  {claim.reimbursement.status === "AWAITING_CHANNEL" ? <div className={styles.channelList}>
                    {canLinkPayroll ? <form action={selectClaimChannelAction} className={`${styles.channelOption} ${styles.payrollChannel}`}>
                      ...
                      {!eligiblePayrollRuns.length ? <small className={styles.channelNote}>{data.payrollRuns.length ? "This employee is not included in an open payroll draft. Add them in Payroll first." : "Create a payroll draft before using this option."}</small> : null}
                    </form> : null}
        ...
        }) : <div className={styles.empty}><strong>{hasFilters ? "No claims match these filters" : "No claims to review"}</strong><span>{hasFilters ? "Clear the filters or try another search." : "New employee claims will appear here."}</span>{hasFilters ? <Link href="/team/claims">Clear filters</Link> : null}</div>}</div>
      </section>

      {canManage ? <section className={styles.settingsPanel}>
        <div className={styles.settingsSummary}><div><p className={styles.eyebrow}>SETTINGS</p><h2>Claim categories</h2><span>{data.categories.length} categories · limits, receipts and effective dates</span></div><Link className={styles.secondaryButton} href={manageCategories ? "/team/claims" : "/team/claims?manage=categories"}>{manageCategories ? "Close" : "Manage"}</Link></div>
        {manageCategories ? <div className={styles.categoryManager}>
          <div className={styles.categoryToolbar}><div><h3>Claim categories</h3><p>Choose a category to view or update its current policy.</p></div><Link className={styles.primaryLink} href="/team/claims?manage=categories&newCategory=1">+ Add category</Link></div>
          <div className={styles.categoryLayout}>
            <div className={styles.categoryList}>{data.categories.map((category) => { const current = category.revisions.find((revision) => revision.effectiveFrom <= currentDate && (!revision.effectiveTo || revision.effectiveTo >= currentDate)) ?? category.revisions[0]; return <Link key={category.id} className={selectedCategory?.id === category.id ? styles.categoryActive : styles.categoryRow} href={`/team/claims?manage=categories&category=${category.id}`}><span><strong>{category.name}</strong><small>{policySummary(category.nature, current?.maxLineAmount ?? null, current?.mileageRatePerKm ?? null)}</small></span><span><b>{current?.statutoryTreatmentStatus === "VERIFIED_NON_WAGE" ? "Business reimbursement" : "Needs payroll review"}</b><small>{current?.receiptRequired ? "Receipt required" : "Receipt optional"}</small></span></Link>; })}</div>
