...
export default async function StatutorySubmissionPage({ searchParams }: PageProps) {
  ...
  const canManage = context.access.effectiveBusinessRole !== "STAFF" || context.access.permissions.includes("PAYROLL_MANAGE");
    ...
    <main className={`content hr-module-page ${styles.page}`}>
          ...
          return (
            <article className={styles.providerCard} key={provider.id}>
              ...
              <div className={styles.cardActions}>
                {result.ready ? <Link className={styles.primaryButton} href={`/team/payroll/statutory/export?month=${period.value}&provider=${provider.id}`}>Download official file</Link> : <span className={styles.disabledButton}>Complete required fields</span>}
                <small>{STATUTORY_EXPORT_VERSION[provider.id]}</small>
              </div>
              {submission ? <SubmissionWorkflow submission={submission} month={period.value} canManage={canManage} /> : null}
            </article>
          );
        })}
      </section>
      ...
      <section className={styles.panel}>
        ...
        <form action={saveBusinessStatutoryProfileAction} className={styles.companyForm}>
          ...
          <Field disabled={!canManage} label="KWSP employer number" name="epfEmployerNumber" placeholder="Employer registration number" value={profile?.epfEmployerNumber ?? ""} />
          <Field disabled={!canManage} label="PERKESO employer code" maxLength={12} name="perkesoEmployerCode" placeholder="Exactly 12 letters/digits" value={profile?.perkesoEmployerCode ?? ""} />
          <Field disabled={!canManage} label="MyCoID / SSM number" maxLength={20} name="perkesoRegistrationNumber" placeholder="Optional in ASSIST file" value={profile?.perkesoRegistrationNumber ?? ""} />
          <Field disabled={!canManage} inputMode="numeric" label="LHDN employer no. (HQ)" maxLength={10} name="lhdnEmployerNumberHq" placeholder="10 digits" value={profile?.lhdnEmployerNumberHq ?? ""} />
          <Field disabled={!canManage} inputMode="numeric" label="LHDN employer number" maxLength={10} name="lhdnEmployerNumber" placeholder="10 digits" value={profile?.lhdnEmployerNumber ?? ""} />
          {canManage ? <button className={styles.primaryButton} type="submit">Save company registration</button> : null}
        </form>
      </section>
            ...
            return (
              <details className={styles.employeeCard} key={employee.id}>
                ...
                <form action={saveEmployeeSubmissionProfileAction} className={styles.employeeForm}>
                  ...
                  <input name="month" type="hidden" value={period.value} />
                  <label><span>Identity type</span><select defaultValue={employee.statutoryIdentityType ?? ""} disabled={!canManage} name="statutoryIdentityType"><option value="">Select type</option><option value="NEW_IC">New IC / MyKad</option><option value="OLD_IC">Old IC</option><option value="PASSPORT">Passport</option><option value="OTHER">Other</option></select></label>
                  <Field disabled={!canManage} label="Identity number" name="statutoryIdentityNumber" placeholder="No spaces or dashes preferred" value={employee.statutoryIdentityNumber ?? ""} />
                  <Field disabled={!canManage} label="LHDN country code" maxLength={2} name="statutoryCountryCode" placeholder="Passport only, e.g. MY" value={employee.statutoryCountryCode ?? ""} />
                  <Field disabled={!canManage} label="KWSP member number" name="epfMemberNumber" placeholder="EPF member number" value={employee.epfMemberNumber ?? ""} />
                  <Field disabled={!canManage} label="SOCSO / foreign worker no." name="socsoMemberNumber" placeholder="Optional if IC is used" value={employee.socsoMemberNumber ?? ""} />
                  <Field disabled={!canManage} inputMode="numeric" label="Tax Identification Number" maxLength={11} name="taxIdentificationNumber" placeholder="11 digits" value={employee.taxIdentificationNumber ?? ""} />
                  {canManage ? <button className={styles.secondaryButton} type="submit">Save employee profile</button> : null}
                </form>
              </details>
            );
          })}
...
function SubmissionWorkflow({ submission, month, canManage }: { submission: { id: string; status: string; exportedAt: Date; submittedAt: Date | null; resolvedAt: Date | null; submissionReference: string | null; rejectionReason: string | null }; month: string; canManage: boolean }) {
  return (
    <div className={styles.workflow}>
      ...
      {canManage && submission.status === "EXPORTED" ? (
