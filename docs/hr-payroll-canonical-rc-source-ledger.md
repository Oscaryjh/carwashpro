...
| `src/components/staff-pwa/staff-attendance-corrections-v2.tsx` | pre-RED current `9489eb56958e30e3c31d38e0f995ecfb681b4428b3e7b5a1a007e34eca9b462a`; selected result `f42f76d84b69c6b4324639dbdcf1027cd0c65bbae7f2344d86b9d253ef8403e4` | Current canonical → same path | Staff corrections UX | Existing correction archive/projector | **Selected, hunk-only after RED**: approved/rejected final recorded result is now distinct from the preserved employee request | Staff correction final-result and own-status tests | `feat(staff): complete eight-role self-service UAT fixture` | Own membership only; locked records remain immutable |
| `src/lib/staff-pwa/attendance-corrections-v2.ts` | selected result `166030b7648c027215244620beb60877c494ff389f1bdeb3b701e0eb42756a65` | Current canonical → same path | Staff corrections presentation | Canonical correction archive DTO | **Selected, minimal pure helper after RED**: returns recorded results only for final approved/rejected states and never relabels a pending proposal | Pure approved/rejected/pending/null result assertions | `feat(staff): complete eight-role self-service UAT fixture` | Read-only own-record presentation; no correction mutation |
...
| `scripts/prepare-hr-payroll-five-role-uat.ts` | current/old identical `7953809c7b7ba90bce7e726a4926581458e8e4b32b2c142893578a8ecbcb0dab` | Current canonical → unchanged | Local UAT fixture | Existing HR acceptance fixture; local sessions | **Verification-only / not modified**: its six-persona output failed the eight-role contract, so it remains historical evidence and a new narrow fixture supersedes it for Phase 2 | Existing historical fixture only | None | Local/disposable only; never Testing/Production data or credentials |
| `scripts/hr-payroll-eight-role-uat-contract.ts` | `b3864adcb417fb3dfe84a7ea17ae9799f2053dfee3a35c1fbf668353b5aa3756` | New contract | Local UAT fixture | Existing capability names and local URL guard | **Selected after fixture-contract RED**: exact ordered role set and least-privilege direct-role definitions; Payroll Admin excludes statutory submission and payment-file export | Exact role order/count, local URL, Production rejection, credential bound | `feat(staff): complete eight-role self-service UAT fixture` | Local/disposable only; no Testing/Production credentials |
| `scripts/prepare-hr-payroll-eight-role-uat.ts` | `290145baf442f05cc02b481a98faf366b22eb19d011dfc96db3f937b50594bf2` | New fixture wrapper | Local UAT fixture | Existing local core acceptance artifact; app and employee session services | **Selected after fixture-contract RED**: creates Business Owner, Payroll Admin, HR Manager, Branch Manager, Supervisor, Group Owner, Group Manager, and Staff without changing the historical fixture | Contract unit test; actual disposable fixture execution; browser role matrix | `feat(staff): complete eight-role self-service UAT fixture` | Exact loopback DB only; Production rejected; output contains local ephemeral sessions only |
| `tests/unit/hr-payroll-eight-role-uat.test.ts` | `f02734768f7cea6b9207b978ccdc5d8e5a18139125d9aba7c05ef35fbb11fdcc` | New test | Fixture contract | Eight-role contract | **Selected**: proves exact persona order/count and negative environment guards before browser seeding | Unit contract RED/GREEN | `feat(staff): complete eight-role self-service UAT fixture` | No DB or remote access |
| `src/lib/employee-avatar-image.ts` | `8aec4c7a970709cd353ed8d6a7e57a2984f8739dfa8d4785c421d52abf218d6e` | Extracted from current avatar route | Avatar image boundary | Direct Sharp runtime | **Selected after safe AVIF test RED**: isolates the exact existing rotate/resize/WebP pipeline so it can be exercised before and after dependency remediation without changing behavior | In-memory non-malicious AVIF → 512×512 WebP | `test(security): cover safe AVIF image paths` | No payload download or stored exploit fixture |
| `tests/unit/employee-avatar-avif-security.test.ts` | `4667fcc5d3dc21526cc9ef03908d05bbb792cbc081126218bcf4ead8b40c6cde` | New test | Avatar and Next Image Optimization | Direct avatar helper; Next `optimizeImage`; Sharp | **Selected**: covers both required AVIF execution paths with generated in-memory pixels and no malicious content | Pre/post-upgrade AVIF metadata and dimensions | `test(security): cover safe AVIF image paths` | Safe synthetic in-memory fixture only |
