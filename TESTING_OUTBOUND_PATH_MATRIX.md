...
| WhatsApp | send API, manual send action, closing/reminder queue, retry worker → connector client | Queue requires explicit `mock`; direct connector `/send` denied before `fetch`, even if queue is bypassed | `testing-outbound-boundary.test.ts`, `notification-queue-worker-send-mode.test.ts` |
