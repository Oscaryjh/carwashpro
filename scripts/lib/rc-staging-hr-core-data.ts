...
import { createStagingAuthorizer } from "./rc-staging-fixture-mfa";
...
export async function seedStagingHrCore(prisma: PrismaClient, password: string) {
  ...
  const fixture = await prisma.$transaction(async (tx) => {
    const business = await tx.business.create({
      data: {
        name: BUSINESS_NAME,
        slug: HR_PAYROLL_UAT_SYNTHETIC_BUSINESS_SLUG,
    ...
    for (const scenario of scenarios) {
      ...
      // A revoked historical row is needed by the synthetic correction's FK.
      // Never issue an active Staff session or an OTP before browser verification.
      await tx.employeeSession.create({
        data: {
          ...
          refreshTokenHash: digest(randomUUID()),
          createdAt: new Date("2026-09-01T00:00:00Z"),
          lastActiveAt: new Date("2026-09-01T00:00:00Z"),
          expiresAt: new Date("2026-09-02T00:00:00Z"),
          revokedAt: new Date("2026-09-01T01:00:00Z"),
          revokeReason: "RC_STAGING_SYNTHETIC_HISTORICAL_REFERENCE_ONLY",
          userAgent: "RC Staging synthetic historical correction",
        },
      });
  ...
  // Amounts are explicit synthetic evidence, confirmed through real MFA/services.
  const manualPcbAmounts: Record<string, string> = {
    "CORE-A": "0.00", "CORE-B": "0.00", "CORE-C": "0.00",
    "CORE-D": "0.00", "CORE-E": "0.00", "CORE-F": "0.00",
  };
  const authorizer = await createStagingAuthorizer(prisma, fixture.business.id, fixture.owner.id, password);
  ...
  const reviewer = await prisma.user.create({ data: { businessId: fixture.business.id, branchId: fixture.branch.id,
    ...
    loginEnabled: true, status: "active" } });
  const reviewerAuthorization = await createStagingAuthorizer(prisma, fixture.business.id, reviewer.id, password);
  const stepUp = await reviewerAuthorization.authorize("PAYROLL_FINALIZE", run.id);
