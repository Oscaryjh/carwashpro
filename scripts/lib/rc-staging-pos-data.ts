...
// Preserve the approved POS master records and synthetic trace; no bank provider calls.
export async function seedStagingPos(prisma: PrismaClient, password: string) {
...
async function addSalonFinancialTrace(salon: Awaited<ReturnType<typeof seedSalon>>) {
  ...
  const salon = await seedSalon(hash);
  await seedAuto(hash);
